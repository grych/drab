...
Drab.on_load(function(resp, drab) {
  ...
  // extract information from all drabbed nodes and store it
  var d = window.__drab
  d.amperes = {}
  d.amperes.injected = {}
  drabbed.forEach(function(node) {
    if (node.getAttribute("__drabbed") == "ampere") {
      // injected spans
      var a = node.getAttribute("drab-assigns")
      a.split(/\s+/).forEach(function(assign) {
        if (!(assign in d.amperes.injected)) {
          d.amperes.injected[assign] = []
        }
        d.amperes.injected[assign].push({
