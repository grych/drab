defmodule Drab do
  ...
  All Drab functions (callbacks and event handlers) should be placed in a module called a 'commander'. It is very
  similar to controller, but it does not render any pages - it works with the live page instead. For example,
  pages generated by `DrabExample.PageController` are handled by commander with the corresponding name, in this
  case `DrabExample.PageCommander`:
